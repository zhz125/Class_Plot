...
#  @output: intermediate_files/features_included_in_plots.csv, which includes all the features that are included in the plot
#           intermediate_files/format_toplot.csv, which will be used directly into plot functions
...
def get_plot_table(t_test_f,fold_change_f):
    ...
    input.dropna(subset = ['xlogp','Fold Change'],inplace=True)
    ...
    input.to_csv('intermediate_files/features_included_in_plots.csv',columns=['FeatureID','Fold Change','p.value'])
